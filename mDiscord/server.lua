...
exports('SetGroup', function (autor, id, groups, permissions)
    if GetPlayerName(id) then
        ...
        if (xPlayer) then
            ...
            xPlayer.showNotification("☂️ : ~r~Vous avez reçus un nouveaux rôle en jeux ~s~("..groups..", "..permissions..")~r~, vous aller être deconnecté dans quelque secondes !")
            local embed = {
...
exports('GetPlayers', function ()
    ...
    TriggerEvent('esx:getSharedObject', function(ESX)
        ...
        embed.title = '`'..#Players..'` Joueurs connectée(s)'
        if next(Players) then
            embed.fields = {
                {name = 'Nom du joueur', inline = true, value = ''},
                {name = 'Licence', inline = true, value = ''},
                {name = "Discord", inline = true, value = ''},
                {name = "Cfx", inline = true, value = ''}
            }
            for i = 1, #Players do
                local xPlayer = ESX.GetPlayerFromId(Players[i])
                local discordID = GetDiscord(Players[i])
                local fivemID = GetFivem(Players[i])
                embed.fields[1].value = "("..Players[i]..") - "..embed.fields[1].value..GetPlayerName(Players[i])..''
                embed.fields[2].value = embed.fields[2].value..xPlayer.identifier
                embed.fields[3].value = embed.fields[3].value..(discordID == false and "<Inconnu>" or discordID)
                embed.fields[4].value = embed.fields[4].value..(fivemID == false and "<Inconnu>" or fivemID)
            end
        end
        TriggerEvent('DiscordAPI:SendEmbed', embed)
    end)
...
exports('revive', function (autor, id)
    if GetPlayerName(id) then
        local xPlayer = ESX.GetPlayerFromId(id)
        TriggerClientEvent('EMS:ReviveClientPlayer', id)
        xPlayer.showNotification("~p~[STAFF]~s~ \nTu viens d'être ~g~réanimé~s~ via discord par un membre du staff \nBon jeu à toi !")
        local embed = {
            color = "#1bd411", -- blue
...
exports('ReviveAll', function (autor)
    TriggerEvent('esx:getSharedObject', function(ESX)
        local Players = ESX.GetPlayers()
        for i = 1, #Players do
            TriggerClientEvent('EMS:ReviveClientPlayer', Players[i])
        end
        if next(Players) then
            local embed = {
                color = "#1bd411", -- blue
